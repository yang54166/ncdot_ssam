...
export default function WorkOrdersListViewQueryOption(context) {
    ...
    if (searchString) {
        //Standard order filters (required when using a dataQueryBuilder)
         ...
         filters.push(`substringof('${searchString.toLowerCase()}', tolower(HeaderFunctionLocation))`);
         filters.push(`substringof('${searchString.toLowerCase()}', tolower(FunctionalLocation/FuncLocDesc))`);
        
         filters.push(`substringof('${searchString}', tolower(OrderType))`);
