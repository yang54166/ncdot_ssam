...
export default function WorkOrderOperationsListViewQueryOption(context) {
    ...
    if (searchString) {
        ...
        //hlf -- add search floc and floc desc
        filters.push(`substringof('${searchString.toLowerCase()}', tolower(WOHeader/HeaderFunctionLocation))`);
        filters.push(`substringof('${searchString.toLowerCase()}', tolower(WOHeader/FunctionalLocation/FuncLocDesc))`);
    ...
    //hlf -- from assigned to technician 
    } else if (CommonLibrary.getStateVariable(context, 'FromOperationsAssignedList')) { // from assigned operations
        queryBuilder = Constants.FromWOrkOrderOperationListQueryOptions(context);
        let personNbr = CommonLibrary.getPersonnelNumber();
        // get prabha number
        personNbr = '01555611';

        queryBuilder.filter("PersonNum eq '" + personNbr + "'");
        if (filter) {
            queryBuilder.filter().and(filter);
        }
