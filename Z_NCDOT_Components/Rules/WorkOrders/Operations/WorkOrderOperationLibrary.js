...
export class OperationLibrary {
    ...
    static getCreateUpdateLinks(pageProxy) {
        ...
        if (libCommon.isDefined(libOperationControl.getWorkOrder(pageProxy))) {
            ...
            //hlf - link to employee
            try {
                let person = pageProxy.evaluateTargetPath('#Control:Z_PersonAssigned/#SelectedValue');
                if (person) {
                    links.push({
    ...
    static getDeleteLinks(pageProxy) {
        ...
        //hlf - link to employee
        if (pageProxy.binding.PersonNum) {
            try {
                let person = pageProxy.evaluateTargetPath('#Control:Z_PersonAssigned/#SelectedValue');
                if (!person) {
                    links.push({
                        ...
                        {
                            'EntitySet': 'Employees',
                            'ReadLink': `Employees('${pageProxy.binding.PersonNum}')`
                        },
                    });
                }
            }
...
export class OperationEventLibrary {
    ...
    static createUpdateOnPageLoad(pageProxy) {
        if (!pageProxy.getClientData().LOADED) {
            ...
            if (onCreate) {
                ...
                //hlf will have to apply a change here
                this.setDefaultValues(pageProxy);
            } else {
                //hlf
                ///"/SAPAssetManager/Rules/WorkOrders/Operations/WorkOrderOperationPersonNum.js",
                ...
                //hlf - maintenance activity type
                let actType = formCellContainerProxy.getControl('Z_MaintActType');
                let isLocal = libCommon.isCurrentReadLinkLocal(pageProxy.binding['@odata.readLink']);
                   if (!isLocal){
                ...
                //hlf - set up query for function area
                  ...
                  //if (!isLocal || pageProxy.binding.ZZTask){
                  if (pageProxy.binding.ZZTask){
                        functionArea.setEditable(false);
                        wbs.setEditable(false);
                  }
                  if (pageProxy.binding.ZZwbs){
                ...
                switch (name) {
                    ...
                    case 'WorkOrderLstPkr': { //Handle enable/disable of equip/floc based on order type of parent WO
                        ...
                        if (woNumReadLink) { //Read defaults from work order header for this new operation
                            return controlProxy.read('/SAPAssetManager/Services/AssetManager.service', woNumReadLink, ['MaintenanceActivityType','OrderType', 'PlanningPlant', 'HeaderEquipment', 'HeaderFunctionLocation', 'OrderDescription'], '').then(result => {
                                if (result && result.length > 0) {
                                    ...
                                    //hlf - NCDOT - add maintenance activity type here
                                    let actType = formCellContainer.getControl('Z_MaintActType');
                                    actType.setValue(woRow.MaintenanceActivityType);
                                    ...
                                    return controlProxy.read('/SAPAssetManager/Services/AssetManager.service', 'OrderTypes', [], `$filter=(OrderType eq '${woRow.OrderType}' and PlanningPlant eq '${woRow.PlanningPlant}')`).then(function (myOrderTypes) {
                                        if (myOrderTypes && myOrderTypes.length > 0) {
                                            ...
                                            let ckValue = record.ControlKey;
                                            if (ckValue === '') {
                                                ckValue = 'PM01';
                                            }
                                            ControlKeyLstPkr.setValue(ckValue);
                                        }
    ...
    static setDefaultValues(pageProxy) {
        ...
        parentWorkOrderPromise.then(parentWorkOrder => {
            parentWo = parentWorkOrder;

            // set the defaut value from Parent - WorkOrder
            //hlf -- this is a bug here - control key should not be set to equal order type but should get the default value 
            //       from the controk key assigned to the selected order type
            ...
            .then(parentWorkOrder => {
                let entity = `OrderTypes(PlanningPlant='` + parentWorkOrder.PlanningPlant + `',OrderType = '` + parentWorkOrder.OrderType + `')`;
                return pageProxy.read('/SAPAssetManager/Services/AssetManager.service', entity, [], '').then(result => {
                    if (result.length > 0) {
                        // set the defaut value from Parent - WorkOrder
                        //hlf -- this is a bug here - control key should not be set to equal order type but should get the default value 
                        //       from the controk key assigned to the selected order type
                        let defaultck = result.getItem(0).ControlKey;
                        if (!result.getItem(0).ControlKey) {
                            defaultck = 'PM01';
                        };

                        controlKeyCtrl.setValue(defaultck);
                    }
                    //set the 'SetValueFromRule' flag of each control to true
                    controlKeyCtrl.getClientData().SetValueFromRule = true;
                    ...
                    .then(ra => {
                        //hlf - set initial values from work order 
                        let wbsCtrl = pageProxy.getControl('FormCellContainer').getControl('Z_wbsListPkr');
                        let faCtrl = pageProxy.getControl('FormCellContainer').getControl('Z_functionAreaLstPkr');
                        let actType = pageProxy.getControl('FormCellContainer').getControl('Z_MaintActType');

                        actType.setValue(parentWorkOrder.MaintenanceActivityType);
                        wbsCtrl.setValue(parentWorkOrder.ZZwbs);
                        faCtrl.setValue(parentWorkOrder.ZZFunctionalArea);
                        faCtrl.setEditable(true);

                        let wbs = `Z_wbs_Elements('` + parentWorkOrder.ZZwbs + `')`;

                        return pageProxy.read('/SAPAssetManager/Services/AssetManager.service', wbs, [], '$select=Z_Phase').then(result => {
                            if (result) {
                                let phase = result.getItem(0).Z_Phase;

                                let functionAreaTargetSpecifier = faCtrl.getTargetSpecifier();
                                let filter = "$filter= Z_Phase eq '" + phase + "'";
                                let orderBy = '$orderby=Z_FunctionalArea';
                                functionAreaTargetSpecifier.setQueryOptions(filter + '&' + orderBy);

                                faCtrl.setTargetSpecifier(functionAreaTargetSpecifier);
                            }
                        });
...
export class OperationConstants {
    // hlf - add expand to WOHeader/FunctionalLocation
    static FromWOrkOrderOperationListQueryOptions(context, useDataQuery = true) {
        let expands = 'WOHeader/Notification/HeaderLongText,WOHeader/FunctionalLocation,WOObjectList_Nav,Tools,OperationMobileStatus_Nav,OperationLongText,WOHeader,UserTimeEntry_Nav,WOHeader/WOPriority,Employee_Nav,WOOprDocuments_Nav/Document';
    ...
    static OperationListQueryOptions(context) {
        ...
        const expandFields = ['WOHeader/Notification/HeaderLongText', 'WOHeader/FunctionalLocation', 'WOObjectList_Nav', 'Tools', 'OperationMobileStatus_Nav', 'OperationLongText', 'WOHeader', 'WOHeader/OrderMobileStatus_Nav', 'WOHeader/UserTimeEntry_Nav', 'UserTimeEntry_Nav', 'WOHeader/WOPriority', 'Employee_Nav', 'WOOprDocuments_Nav/Document'];
        if (userFeaturesLib.isFeatureEnabled(context, context.getGlobalDefinition('/SAPAssetManager/Globals/Features/QM.global').getValue())) {
