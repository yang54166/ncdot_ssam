...
export default function CrewTimesheetOvertimeHours(context) {
    ...
    return context.read('/SAPAssetManager/Services/AssetManager.service', context.binding['@odata.readLink'] + '/CrewList', [], '').then(function(result) {    
        catTimeSheets.forEach(function(element) {
            if (libTimesheet.isOvertime(context, element.ActivityType) && element.Date === result.getItem(0).OriginTimeStamp) {
                if (!hoursCount) {
                    hoursCount = 0;   
                }                
