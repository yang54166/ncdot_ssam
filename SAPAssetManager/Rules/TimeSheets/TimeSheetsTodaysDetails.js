    ...
    return TimeSheetData(context, date).then((data) => {
        return context.read('/SAPAssetManager/Services/AssetManager.service', 'CatsTimesheetOverviewRows', [], "$filter=Date eq datetime'" + data.Date + "'").then(function(results) {
            if (results && results.getItem(0)) {
                ...
                let action = userFeaturesLib.isFeatureEnabled(context, context.getGlobalDefinition('/SAPAssetManager/Globals/Features/Crew.global').getValue()) ? '/SAPAssetManager/Actions/Crew/TimeSheets/TimeSheetDetailsNav.action' : '/SAPAssetManager/Actions/TimeSheets/TimeSheetEntryDetailsNav.action';
