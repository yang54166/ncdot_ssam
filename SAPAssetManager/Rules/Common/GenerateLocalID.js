 ...
 * @param {Int?} extraOffset optional additional ammount to add to a ID
 */

export default function GenerateLocalID(context, entitySet, property, padString, queryOpts = '', prefix, sortProperty, extraOffset=0) {
    ...
    return context.read('/SAPAssetManager/Services/AssetManager.service', entitySet, [], `$select=${property}&$orderby=${sortProperty} desc&$top=1` + queryOpts).then(function(result) {
        // If we have more than zero entries already...
        if (!validation.evalIsEmpty(result.getItem(0)) && !validation.evalIsEmpty(result.getItem(0)[property])) {
            let newID = !isNaN(result.getItem(0)[property].substring(prefix.length)) ? parseInt(result.getItem(0)[property].substring(prefix.length)) : 0;
            // Pad the output. Prepend LOCAL_ if it's a header, and use appropriate padding based on whether or not this is a header
            return PadNumber(newID + extraOffset + 1, padString, prefix);
        // Else we have zero entries and we're adding the first one
        } else {
            // Pad the output. Prepend LOCAL_ if it's a header, and use appropriate padding based on whether or not this is a header
            return PadNumber(1+extraOffset, padString, prefix);
        }
    });
}
