...
export default function NotificationDetailsPopover(context) {
    ...
    if (!isLocal) {
        return libNotifStatus.isNotificationComplete(context).then(status => {
            if (!status) {
                return context.executeAction('/SAPAssetManager/Actions/Notifications/NotificationDetailsPopover.action');
            }
            return '';
        });
    }
