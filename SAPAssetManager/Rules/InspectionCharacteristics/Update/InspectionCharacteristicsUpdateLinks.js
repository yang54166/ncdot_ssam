...
export default function InspectionCharacteristicsUpdateLinks(context) {
    ...
    if (!libVal.evalIsEmpty(context.binding.InspectionCode_Nav) && !libVal.evalIsEmpty(libCom.getControlProxy(context, 'QualitativeValue').getValue()[0])) {
        links.push({
    ...
    } else if (!libVal.evalIsEmpty(context.binding.InspectionCode_Nav) && !libVal.evalIsEmpty(libCom.getControlProxy(context, 'QualitativeValueSegment').getValue()[0])) {
        links.push({
            'Property': 'InspectionCode_Nav',
            'Target':
            {
